...
int main(int argc, char *argv[])
{
    ...
    if (argc < 4 || argc > 5)
    {
        std::cerr << "Usage: " << argv[0] << " <model_path> <image_path> <label_json_path> [enable_profiling=1]" << std::endl;
        return 1;
    }
    ...
    // Create profiler for detailed operation profiling
    ...
    if (enable_profiling && profiler) {
        ...
        std::cout << "[INFO] Running " << num_runs << " iterations for profiling..." << std::endl;
    ...
    if (enable_profiling && profiler) {
        ...
        std::string profile_filename = "detailed_profile_results_" + 
            std::to_string(std::chrono::duration_cast<std::chrono::milliseconds>(
                std::chrono::system_clock::now().time_since_epoch()).count()) + 
            ".txt";
        
        std::ofstream profile_file(profile_filename);
        if (profile_file.is_open()) {
            profile_file << "=== Detailed TensorFlow Lite Profiling Results ===" << std::endl;
            profile_file << "Model: " << model_path << std::endl;
            profile_file << "Image: " << image_path << std::endl;
            profile_file << "Delegate: XNNPACK (" << (delegate_applied ? "Applied" : "Not Applied") << ")" << std::endl;
            profile_file << "Profiling runs: 10" << std::endl;
            profile_file << "=============================================" << std::endl;
            profile_file << summary_output << std::endl;
            
            profile_file.close();
            std::cout << "\n[INFO] Detailed profiling results saved to: " << profile_filename << std::endl;
        }
    }
